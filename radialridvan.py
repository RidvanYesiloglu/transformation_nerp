...
def radialrecon(img, nproj):
    ...
    kdata = project_radial(img, ktraj, im_size, grid_size)
    image_blurry_numpy, image_sharp_numpy = backproject_radial(kdata, ktraj, img, im_size, grid_size, plot=False)   
    
    iminds = [0,30]
    fig, axs = plt.subplots(len(iminds), 2, figsize=(5*len(iminds),10))
    for i in range(len(iminds)):
        axs[i,0].imshow(np.abs(img[0,...,iminds[i]]), cmap='gray')
        axs[i,0].axis('off')
...
image = torch.from_numpy(np.expand_dims(np.load('data73/ims_tog.npy')[41], (0)))#.permute((2,0,1)).unsqueeze(1)
...
ax[0].imshow(np.abs(image_blurry_numpy)[1], cmap='gray')
ax[1].imshow(np.abs(image_sharp_numpy)[1], cmap='gray')
plt.show()


imageio.mimsave('73radialsamplings_torch_1.gif', [radialrecon(image, nproj) for nproj in (list(range(2,12))+list(range(15,101,5)))], fps=0.7)
