...
def project_radial(image, ktraj, im_size, grid_size):
    ...
    kdata = nufft_ob(image, ktraj, norm='ortho')
    return kdata
...
def backproject_radial(kdata, ktraj, image, im_size, grid_size, plot=False):
    ...
    image_blurry = adjnufft_ob(kdata, ktraj, norm=None)
    # method 2: use density compensation
    dcomp = tkbn.calc_density_compensation_function(ktraj=ktraj, im_size=im_size)#.unsqueeze(-1)
    image_sharp = adjnufft_ob(kdata * dcomp, ktraj, norm=None)   
    ...
    if plot:
        plt.figure(0)
        plt.imshow(np.absolute(image_blurry_numpy)[0])
        plt.gray()
        plt.title('blurry image')
        
        plt.figure(1)
        plt.imshow(np.absolute(image_sharp_numpy)[0])
        plt.gray()
